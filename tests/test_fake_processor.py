...
def test_make_dict_from_wallet_list():
    ...
    wallet_dict = make_dict_from_wallet_list(
        wallet_list=[walletETH, walletOMG])
...
def test_execute_order_limit_buy():
    ...
    result_order, result_wallets = execute_order(
        order=order, wallets=[
            walletOMG, walletETH], ticker=ticker, fee_percent=0.002)
...
def test_process_orders_success():
    try:
        ...
        save_wallet_to_file(
            filename=wallets_filename, wallets=[
                walletETH, walletOMG])

        current_ticker = Ticker(Pair.OMGETH, 0.00309)
        process_orders(
            ticker=current_ticker,
            orders_file_name=orders_filename,
            wallets_file_name=wallets_filename)

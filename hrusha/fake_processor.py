...
def process_orders(ticker: Ticker, orders_file_name: str='current_orders.csv', wallets_file_name: str = 'wallets.csv'):
    processed_orders = []
    ...
    for order in orders:
        if order.pair == ticker.pair \
            ...
            result_order, wallets = execute_order(order=order, wallets=wallets, ticker=ticker, fee_percent=0.02)
...
def execute_order(order: StrategyOrder, wallets: List[Wallet], ticker: Ticker, fee_percent: float):
